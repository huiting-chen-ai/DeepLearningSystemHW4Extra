...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        Q = ops.transpose(Q.reshape((batch_size, queries_len, self.num_head, self.dim_head)), (1, 2))
        K = ops.transpose(K.reshape((batch_size, queries_len, self.num_head, self.dim_head)), (1, 2))
        V = ops.transpose(V.reshape((batch_size, queries_len, self.num_head, self.dim_head)), (1, 2))
        result, _ = self.attn(Q, K, V)
        result = ops.transpose(result, (1, 2)).reshape((batch_size, queries_len, self.num_head*self.dim_head))
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transformerlayer = Sequential(*[TransformerLayer(q_features=embedding_size, num_head=num_head, 
